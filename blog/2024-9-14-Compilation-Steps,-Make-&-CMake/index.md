...
است. این ابزار برای management dependency استفاده می شود.
...
ماکروها، دیفاین ها و اینکلودها و حذǿ
کامنت هاست.

برای مثال، اگر یک hpp.file داشته باشیم و آن
را در cpp.file اینکلود کنیم، می بینیم که در
i.file کل محتوای hpp.file به جای دستور
include# آمده است.
...
در این فاز، چندین file object یعنی فایل o. با
...
وقتی می زنیم: out.file o- cpp.file ++g، همه این مراحل به صورت یکجا طی می شوند!
وقتی با پروژه ای چند فایلی کار داریم، به جای اینکه با هر تغییر، کل این مراحل برای همه فایل ها طی شوند (که باعث افزایش
چشم گیر زمان کامپایل می شود)، هر کدام از فایل های سورس (که به آنها unit translation هم گفته می شود) را در ابتدا تبدیل به
file object می کنیم (فایل o.)، و در نهایت آنها را لینک می کنیم.
...
نیست. وقتی یک فایل تغییر می کند، بدΟهیست که به کامپایل
...
در Makefile، برای هر فایل سورس (unit translation) همه
dependencyهای آن را می نویسیم. برنامه Make با توجه به
تارΟخ تغییر فایل ها، اگر یک فایلی نیازمند کامپایل مجدد بود،
آن را انجام می دهد.
...
شروع شود. در غیر این صورت با ارور separator missing
مواجه می شویم.
...
هر file object به یک cpp و چندین hpp وابسته است.
اینجا هدف (target) ساخت o.file است که به 3 فایل
جلویش وابسته است. در صورت جدیدتر بودن یکی از آنها از
o.file، دستور زیرش اجرا می شود که فایل را کامپایل می کند.

## متغیرهای اتوماتیک (Variables Automatic)
...
با استفاده از آنها، قوانین تولید file objectها به شکل زیر
نوشته می شوند:
...
برای دسترسی به مقدار این متغیر، از expansion variable
استفاده می کنیم:
...
می شوند استفاده می شود. تابع out-filter برعکس این کار را
انجام می دهد و آنهایی که مچ هستند را حذف می کند.

مثال زیر cpp.main و همه فایل هایی که با o. تمام می شوند را
از VAR حذف می کند:
...
تابع patsubst که مخفف substitution pattern است، به شکل
زیر است:
...
خروجی این تابع cpp.file و cpp3.file است.
...
اگر در line-command جلوی دستور make نام target-ای را
...
نام PHONY. قرار می دهیم. مانند قانون help-print که بالاتر
...
همانطور که معلوم است، System Buildها الزاما
.نیستند cross-platform

در یک پروژه که افراد زیادی روی آن کار می کنند، معمولا
نمی شود همه را محدود به یک سری ابزار خاص برای کد
زدنشان کرد. مثلا یکی روی VS کار می کند (که از solution file
...
txt.CMakeLists نوشته می شود) داشت.
...
نوشتن txt.CMakeLists، با اجرای برنامه می توانیم فایل های
...
prerequisiteهای file objectها نیست.
...
داخل cpp.foo یک تابع تعریف شده که declaration آن در
hpp.foo قرار دارد. در نهایت این تابع توسط main صدا زده
...
با این کار، فایل های system build پلتفرم فعلی در فولدر
کنونی (build (با استفاده از txt.CMakeLists فولدر قبلی (..)
ساخته می شود.
